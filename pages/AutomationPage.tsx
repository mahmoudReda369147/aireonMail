...
export const AutomationPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const nodes = [
      { id: '1', label: t ? t('automation.new_email_arrives') : 'New Email Arrives', type: 'trigger', x: 50, y: 150 },
      { id: '2', label: t ? t('automation.priority_condition') : 'Priority > 80?', type: 'condition', x: 250, y: 150 },
      { id: '3', label: t ? t('automation.auto_reply_gemini') : 'Auto Reply (Gemini)', type: 'action', x: 450, y: 50 },
      { id: '4', label: t ? t('automation.move_to_urgent') : 'Move to Urgent', type: 'action', x: 450, y: 250 },
    ];
  ...
  return (
    <div className="p-8 h-full flex flex-col">
       <h2 className="text-3xl font-bold mb-6 text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400">{t('automation.title')}</h2>
       <div className="bg-glass border border-glass-border rounded-3xl flex-1 flex flex-col backdrop-blur-md overflow-hidden">
            <div className="p-4 border-b border-glass-border flex gap-3">
                 <Button>{t('automation.add_trigger')}</Button>
                 <Button variant="secondary">{t('automation.add_action')}</Button>
            </div>
