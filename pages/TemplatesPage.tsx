...
export const TemplatesPage: React.FC = () => {
    ...
    const { requestConfirm, t } = useAppContext();
    ...
    const handleDelete = (id: string) => {
        ...
        requestConfirm({
            ...
            message: t('templates.delete_message').replace('{templateName}', templateName),
            variant: 'danger',
            onConfirm: async () => {
                try {
                    await deleteTemplateMutation.mutateAsync(id);
                    showToast(t('templates.template_deleted_success'), "success");
                } catch (error) {
                    console.error('Failed to delete template:', error);
                    showToast(error instanceof Error ? error.message : t('templates.failed_delete_template'), "error");
                }
            }
        });
    };
        ...
        return (
            <div className="h-full flex items-center justify-center text-slate-500">
                <div className="text-center">
                    ...
                    <p className="text-xl mb-2">{t('templates.failed_to_load')}</p>
                    <p className="text-sm mb-4">{t('templates.try_again_later')}</p>
                    <Button onClick={() => refetch()}>{t('templates.retry')}</Button>
                </div>
            </div>
        );
    }
        ...
        <div className="h-full flex flex-col p-8 max-w-7xl mx-auto overflow-y-auto custom-scrollbar w-full">
            <div className="flex items-center justify-between mb-8 w-full">
                <div>
                    ...
                    <p className="text-slate-400 mt-2">{t('templates.description')}</p>
                </div>
                <Button icon={Plus} onClick={handleCreate}>{t('templates.create_template')}</Button>
            </div>
            ...
            {isLoading ? (
                <div className="flex-1 flex items-center justify-center">
                    <div className="text-center text-slate-500">
                        ...
                        <p>{t('templates.loading_templates')}</p>
                    </div>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {templates.length === 0 ? (
                         <div className="col-span-full flex flex-col items-center justify-center p-12 text-slate-500 border border-dashed border-glass-border rounded-3xl bg-glass">
                             ...
                             <p>{t('templates.no_templates_found')}</p>
                         </div>
                    ) : (
                        templates.map(template => (
                            <div key={template.id} className="bg-glass border border-glass-border rounded-3xl p-6 flex flex-col shadow-xl hover:border-slate-600 transition-all group relative">
                                <div className={`absolute top-4 ${t('app.dir') === 'rtl' ? 'left-4' : 'right-4'} flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10`}>
                                    <button onClick={() => handleEdit(template)} className="p-2 bg-black/50 hover:bg-fuchsia-500 text-white rounded-lg backdrop-blur-sm transition-colors" title={t('templates.edit')}><Edit3 className="w-4 h-4" /></button>
                                    <button onClick={() => handleDelete(template.id)} className="p-2 bg-black/50 hover:bg-red-500 text-white rounded-lg backdrop-blur-sm transition-colors" title={t('templates.delete')}><Trash2 className="w-4 h-4" /></button>
                                </div>
                                ...
                                <p className="text-sm text-slate-400 mb-1 font-medium truncate"><span className="text-slate-500">{t('templates.subject')}</span> {template.subject}</p>
                                ...
                                <div className="mt-auto">
                                    <Button className="w-full" icon={Send} onClick={() => handleUse(template)}>{t('templates.use_template')}</Button>
                                </div>
                            </div>
                        ))
                    )}
